...
const modes = [
  {
    ...
    label: "Paraphrase",
    description:
      "Rewrite with fresh wording while keeping the same meaning.",
  },
  {
    value: "simplify",
    label: "Simplify",
    description:
      "Make complex writing clearer and easier to understand.",
  },
  {
    value: "formal",
    label: "Formal",
    description:
      "Create polished wording for professional communication.",
  },
  {
    value: "casual",
    label: "Casual",
    description:
      "Make the message more natural and conversational.",
  },
  {
    value: "persuasive",
    label: "Persuasive",
    description:
      "Make the same message more convincing without inventing facts.",
  },
  {
    value: "expand",
    label: "Expand",
    description:
      "Add clarity using only ideas already present in the source.",
  },
];
...
export default function TextRewriterPage() {
  ...
  const handleRewrite = async () => {
    ...
    try {
      ...
      if (!nextOutput) {
        throw new Error("Empty rewritten text received");
      }
      ...
      setError(
        err.response?.data?.message ||
          "Text rewriting failed. Please check the AI provider and try again."
      );
    } finally {
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      setError(
        "Unable to copy the rewritten text. Please copy it manually."
      );
    }
  };
    ...
    <DashboardLayout>
      <div className="mb-6">
        ...
        <DashboardPageHeader
          ...
          title="Text Rewriter"
          description="Rewrite, paraphrase, simplify, or transform your text while preserving the original meaning."
        />
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
        ...
        <Card className="p-5 sm:p-6">
          ...
          <textarea
            ...
            placeholder="Paste an email, message, paragraph, report section, or any text you want to rewrite..."
            rows={12}
          ...
          <div className="mt-2 flex justify-between text-[11px] text-slate-600">
            ...
            <span>
              Maximum {MAX_LENGTH.toLocaleString()}
            </span>
          </div>
          ...
          <p className="mt-4 text-xs leading-5 text-slate-600">
            Selected: {selectedMode.label}. AI-BOS preserves
            the source meaning and does not invent facts.
          </p>
        </Card>
            ...
            {isProcessing ? (
              ...
              <div className="flex min-h-[420px] items-center justify-center text-center">
                <div className="max-w-md">
                  ...
                  <p className="mt-2 text-sm leading-6 text-slate-500">
                    Paste your text, choose a rewrite style,
                    select a tone, and let AI-BOS transform the
                    writing while keeping the original intent.
                  </p>
                </div>
              </div>
            )}
